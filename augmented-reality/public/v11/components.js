const characterMoveComponent = {
    ...
    tick(time, timeDelta) {
      ...
      if (startPositionRaw) {
        ...
        if (this.el.getAttribute("animation-mixer").clip !== "bite" && this.el.getAttribute("animation-mixer").clip !== "roar" && this.el.getAttribute("animation-mixer").clip !== "attack_tail") {
            this.el.setAttribute('animation-mixer', {
                clip: 'idle',
                loop: 'repeat',
                crossFadeDuration: 0.4,
...
AFRAME.registerComponent('character-action', characterActionComponent)
